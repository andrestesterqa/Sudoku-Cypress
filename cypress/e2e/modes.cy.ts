...
describe('Play mode', () => {
  ...
  it('shows a different number of empty cells', () => {
    ...
    cy.get('.game__cell--filled').should('have.length.greaterThan', 40)
    ...
    cy.get('.game__cell--filled')
      ...
      .as('hardN')
      .then(function (){
        expect(this.easyN, 'easyN').to.be.greaterThan(this.mediumN);
        expect(this.mediumN, 'mediumN').to.be.greaterThan(this.hardN);
      })
    })
